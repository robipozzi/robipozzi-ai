...
# Note, in this lab, the Size feature is in sqft while earlier labs utilized 1000 sqft.  This data set is larger than the previous lab.
# 
# We would like to build a linear regression model using these values so we can then predict the price for other houses - say, 
...
# Above, increasing size also increases price. Bedrooms and floors don't seem to have a strong impact on price. 
...
# </figure>
# The lectures discussed some of the issues related to setting the learning rate $\alpha$. 
...
# This plot is only showing $w_0$ with the other parameters fixed at benign values. 
# In this and later plots you may notice the blue and orange lines being slightly off.

# 
# ### $\alpha$ = 9e-7
print("### Let's try a bit smaller value and see what happens.")
# In[5]:
print("### set alpha to 9e-7")
...
# On the left, you see that cost is decreasing as it should. On the right, you can see that $w_0$ is still oscillating around the minimum, 
# but the cost is decreasing with every iteration rather than increasing. 
...
# On the right, you can see that $w_0$ is approaching the minimum without oscillations. `dj_w0` is negative throughout the run. 
...
# If you are interested in the details of why this is the case, click on the 'details' header below. 
# If not, the section below will walk through an implementation of how to do feature scaling.
...
# </summary>
# 
# Let's look again at the situation with $\alpha$ = 9e-7. This is pretty close to the maximum value we can set $\alpha$ to without diverging. 
...
# </figure>
# 
# Above, while cost is being decreased, its clear that $w_0$ is making more rapid progress than the other parameters due to 
# its much larger gradient.
# 
...
# Notice the difference between `w0` and `w1`,`w2`,`w3` as well as  `dj_dw0` and `dj_dw1-3`. `w0` reaches its near final value 
# very quickly and `dj_dw0` has quickly decreased to a small value showing that `w0` is near the final value. 
# The other parameters were reduced much more slowly.
# 
...
# In this case, $w_0$ is multiplied by 'size(sqft)', which is generally > 1000,  while $w_1$ is multiplied by 'number of bedrooms', 
...
#   rescale each feature by both its minimum and maximum values using (x-min)/(max-min). 
#   Both ways normalizes features to the range of -1 and 1, where the former method works for positive features which is simple 
#   and serves well for the lecture's example, and the latter method works for any features.
...
# where $j$ selects a feature or a column in the $\mathbf{X}$ matrix. $µ_j$ is the mean of all the values for feature (j) 
# and $\sigma_j$ is the standard deviation of feature (j).
# $$
# \begin{align}
...
# Given a new x value (living room area and number of bed-rooms), we must first normalize x using the mean and standard deviation
...
# The plot above shows the relationship between two of the training set parameters, "age" and "size(sqft)". 
...
# the square feet versus w[1], the number of bedrooms before normalizing the features. 
...
# ## Acknowledgments
# The housing data was derived from the [Ames Housing dataset](http://jse.amstat.org/v19n3/decock.pdf) compiled by Dean De Cock 
# for use in data science education.