...
# Your goal is to find a model $f_{w,b}(x) = wx + b$, with parameters $w,b$,  which will accurately 
# predict house values given an input $x$. The cost is a measure of how accurate the model is on the training data.
# 
# The cost equation (1) above shows that if $w$ and $b$ can be selected such that the predictions $f_{w,b}(x)$ match 
# the target data $y$, the $(f_{w,b}(x^{(i)}) - y^{(i)})^2 $ term will be zero and the cost minimized. 
# In this simple two point example, you can achieve this!
# 
...
# It is worth noting that some of the plotting in this course can become quite involved. 
# The plotting routines are provided and while it can be instructive to read through the code to become familiar with the methods, 
# it is not needed to complete the course successfully. The routines are in lab_utils_uni.py in the local directory.
...
# In this case, values of approximately $w=209$ and $b=2.4$ provide low cost. 
...
# In the previous plot, because the $w$ and $b$ dimensions scale differently, this is not easy to recognize. 
# The following plot, where $w$ and $b$ are symmetric, was shown in lecture:
