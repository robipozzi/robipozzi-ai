...
# **Once you have passed this assignment** and want to experiment with any of the non-graded code, you may follow 
# the instructions at the bottom of this notebook._
...
# that embodies the movie tastes of a user. 
...
# This is shown in the diagram above. The equation is an example of predicting a rating for user one on movie zero.

# 
# In this exercise, you will implement the function `cofiCostFunc` that computes the collaborative filtering
# objective function. After implementing the objective function, you will use a TensorFlow custom training loop 
# to learn the parameters for collaborative filtering. The first step is to detail the data set and data structures 
...
# 
# The matrix $Y$ (a  $n_m \times n_u$ matrix) stores the ratings $y^{(i,j)}$. 
...
# We'll also load $\mathbf{X}$, $\mathbf{W}$, and $\mathbf{b}$ with pre-computed values. 
# These values will be learned later in the lab, but we'll use pre-computed values to develop the cost model.
...
# Consider developing the cost function in two steps. First, develop the cost function without regularization. 
...
print(f"Cost (Vectorized implementation): {J:0.2f}")

# Evaluate cost function with regularization 
J = cofi_cost_func_v(X_r, W_r, b_r, Y_r, R_r, 1.5);
print(f"Cost (with regularization) (Vectorized implementation): {J:0.2f}")
...
# 
# In the cell below, you can enter your own movie choices. 
# The algorithm will then make recommendations for you! 
# We have filled out some values according to our preferences, but after you have things working with our choices, 
# you should change this to match your tastes.
...
# The operations involved in learning $w$, $b$, and $x$ simultaneously do not fall into the typical 'layers' offered in the 
# TensorFlow neural network package.  Consequently, the flow used in Course 2: Model, Compile(), Fit(), Predict(), 
...
# This is a very brief introduction to a useful feature of TensorFlow and other machine learning frameworks. 
...
# To predict the rating of movie $i$ for user $j$, you compute $\mathbf{w}^{(j)} \cdot \mathbf{x}^{(i)} + b^{(j)}$. 
...
# This section uses a [Pandas](https://pandas.pydata.org/) data frame which has many handy sorting features.
