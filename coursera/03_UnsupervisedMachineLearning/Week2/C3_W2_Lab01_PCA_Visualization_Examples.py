...
# The second principal component adds an additional 0.76% of the information ("explained variance") 
# that is not stored in the first principal component coordinates.
...
# If you had 2 features (two columns of data) and choose 2 principal components, then you'll keep all the information and 
# the data will end up the same as the original.
...
# Notice how the data are now just on a single line (this line is the single principal component that was used to describe the data; 
# and each example had a single "coordinate" along that axis to describe its location.
...
# The next code will generate a widget where you can see how different ways of compressing this data into 1-dimensional datapoints 
# will lead to different ways on how the points are spread in this new space. 
# The line generated by PCA is the line that keeps the points as far as possible from each other. 
# 
# You can use the slider to rotate the black line through its center and see how the points' projection onto the line 
# will change as we rotate the line. 
# 
# You can notice that there are projections that place different points in almost the same point, and there are 
# projections that keep the points as separated as they were in the plane.
plot_widget()
