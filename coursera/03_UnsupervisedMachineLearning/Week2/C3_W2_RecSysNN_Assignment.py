...
# _**NOTE:** To prevent errors from the autograder, you are not allowed to edit or delete non-graded cells in this lab. 
...
# For example "Toy Story 3" was released in 2010 and has several genres: "Adventure|Animation|Children|Comedy|Fantasy". 
...
# Let's learn a bit more about this data set. The table below shows the top 10 movies ranked by the number of ratings. 
# These movies also happen to have high average ratings. How many of these movies have you watched? 
...
# 
# In the collaborative filtering lab, you generated two vectors, a user vector and an item/movie vector whose dot product 
# would predict a rating. 
...
# about the user and/or movie that may improve the prediction. 
...
# Some of the user and item/movie features are not used in training. In the table above, the features in brackets "[]" 
# such as the "user id", "rating count" and "rating ave" are not included when the model is trained and used.
# Above you can see the per genre rating average for user 2. Zero entries are genre's which the user had not rated. 
...
# Above, the movie array contains the year the film was released, the average rating and an indicator for each potential genre. 
...
# Above, we can see that movie 6874 is an Action/Crime/Thriller movie released in 2003. User 2 rates action movies as 3.9 on average. 
# MovieLens users gave the movie an average rating of 4. 'y' is 4 indicating user 2 rated movie 6874 as a 4 as well. 
...
# This was used in Course 1, Week 2, Lab 5.  Below, the inverse_transform is also shown to produce the original inputs. 
...
# Here we will use [sklean train_test_split](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.train_test_split.html) 
# to split and shuffle the data. 
# Note that setting the initial random state to the same value ensures item, user, and y are shuffled identically.
...
# If the user content was substantially larger than the movie content, 
# you might elect to increase the complexity of the user network relative to the movie network. 
...
#     
# The remainder of the network will be provided. 
# The provided code does not use the Keras sequential model but instead uses the Keras [functional api](https://keras.io/guides/functional_api/). 
...
# After you have tried this on the example user content, feel free to change the user content to 
# match your own preferences and see what the model suggests. 
...
# Let's look at the predictions for "user 2", one of the users in the data set. 
# We can compare the predicted ratings with the model's ratings.
uid = 2 
...
# The model prediction is generally within 1 of the actual rating though it is not a very accurate predictor of 
# how a user rates specific movies. This is especially true if the user rating is significantly different than the user's genre average. 
# You can vary the user id above to try different users. Not all user id's were used in the training set.
...
# For example, if a user has rated "Toy Story 3" highly, one could recommend similar movies by selecting movies 
# with similar movie feature vectors.
# 
...
def sq_dist(a,b):
    ...
    """
    ### START CODE HERE ###
    # You can utilized np.square to square, element-wise, the result of the subtraction. 
    # np.sum can be used to sum the squared elements.
...
#   <summary><font size="3" color="darkgreen"><b>Click for hints</b></font></summary>
#     
# While a summation is often an indication a for loop should be used, here the subtraction can be element-wise in one statement. 
# Further, you can utilized np.square to square, element-wise, the result of the subtraction. np.sum can be used to sum the squared elements.
#     
# </details> 
...
# Once you have a movie model, you can create a set of movie feature vectors by using the model to predict 
# using a set of item/movie vectors as input. `item_vecs` is a set of all of the movie vectors. 
...
# We will make use of [numpy masked arrays](https://numpy.org/doc/1.21/user/tutorial-ma.html) to avoid selecting the same movie. 
...
# Items are not limited to movies. This can be used to recommend any item, books, cars or items that are similar to an item 
